...
open class SweetAlerts: UIViewController {
    ...
    var userAction2:((_ isOtherButton: Bool) -> Void)? = nil
    
    
    var chek :Bool = false
    var titleLbl : UILabel = UILabel()
    let titleLblHeight : CGFloat = 15
    var isTapGestureReg : Bool = true
    
    var lineview = UILabel()
    var klineHeight : CGFloat = 0.5
    ...
    init() {
        ...
        self.view.backgroundColor = UIColor(white: 0.85, alpha: kBakcgroundTansperancy)
    ...
    fileprivate func setupContentView2() {
        contentView.backgroundColor = UIColor(white: 1.0, alpha: 1.0)
        contentView.layer.cornerRadius = 5.0
        contentView.layer.masksToBounds = true
        contentView.layer.borderWidth = 0.5
    ...
    fileprivate func setupbuttonView(){
        ...
        buttonview.backgroundColor = UIColor.init(red: 240/255, green: 240/255, blue: 240/255, alpha: 1)
    }
    ...
    fileprivate func setupTitleLabel() {
        ...
        titleLabel.font = Constatns.Lato_Bold(15)
        titleLabel.textColor = UIColor.colorFromRGB(0x575757)
    }
    
    fileprivate func setupTitleLabel2() {
        titleLbl.text = ""
        titleLbl.numberOfLines = 1
        titleLbl.textAlignment = .center
        titleLbl.font = Constatns.Lato_Bold(15)
    ...
    fileprivate func setupSubtitleTextView() {
        ...
        subTitleTextView.font = Constatns.LatoRegular(13)
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if self.titleLabel.text != nil {
            ...
            contentView.addSubview(titleLabel)
            y += titleHeight + kHeightMargin
        }
        
        
        // lineview
        lineview.frame = CGRect(x: 0, y: y, width: width+20, height: klineHeight)
        buttonview.frame = CGRect(x: 0, y: y, width: width+20, height: 50)
        contentView.addSubview(buttonview)
        ...
        if buttons.count == 2{
            ...
            for i in 0 ..< buttons.count {
                ...
                buttons[i].frame = CGRect(x: buttonX, y: 10, width: 80, height: 30)
                buttonX = buttons[i].frame.origin.x + kWidthMargin + 120
                buttons[i].layer.cornerRadius = 15.0
                buttonview.addSubview(buttons[i])
                self.contentView.addSubview(buttonview)
                buttons[i].addTarget(self, action: #selector(SweetAlerts.pressed(_:)), for: UIControlEvents.touchUpInside)
            }
            
        }else{
            buttons[0].frame = CGRect(x: 110, y: 10, width: 80, height: 30)
            buttons[0].layer.cornerRadius = 15.0
            buttons[0].backgroundColor = Constatns.ThemeColour()
            buttonview.addSubview(buttons[0])
            self.contentView.addSubview(buttonview)
            buttons[0].addTarget(self, action: #selector(SweetAlerts.pressed(_:)), for: UIControlEvents.touchUpInside)
        }
        
        y += kHeightMargin + 20 + 10.0
        
        contentView.frame = CGRect(x: (mainScreenBounds.size.width - kContentWidth) / 2.0, y: (mainScreenBounds.size.height - y) / 2.0, width: kContentWidth, height: y+10)
        contentView.clipsToBounds = true
    }
    
    
    fileprivate func resizeAndRelayout2() {
        ...
        // Title
        if self.titleLbl.text != nil {
            titleLbl.frame = CGRect(x: x, y: 19, width: width, height: titleLblHeight)
            contentView.addSubview(titleLbl)
            y += titleLblHeight + 15
        }
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            subTitleTextView.frame = CGRect(x: x, y: y, width: width, height: 100)
            contentView.addSubview(subTitleTextView)
            y += 100
        }
        ...
        if buttons.count == 2{
            ...
            buttons[0].frame = CGRect(x: 80, y: 10, width: 140, height: 30)
            buttons[0].layer.cornerRadius = 15.0
    ...
    open func showAlert(_ title: String) -> SweetAlerts {
        _ =  showAlert(title,  style: .none)
        return self
    }
    
    open func showAlert(_ title: String, style: AlertStyle) -> SweetAlerts {
       
        let ok = NSLocalizedString("Okay", comment: "")
        
        _ = showAlert(title,  style: style, buttonTitle: ok)
        return self
        
    }
    
    open func showDeactivationAlert(_ title: String, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlerts {
        _ = showAlert(title,  style: style, buttonTitle: buttonTitle,buttonColor: Constatns.ThemeColour())
        userAction = action
        return self
    }
    ...
    open func showAlert(_ title: String, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlerts {
        _ = showAlert(title,  style: style, buttonTitle: buttonTitle,buttonColor: Constatns.ThemeColour())
        userAction = action
        return self
    }
    
    open func showAlert(_ title: String,  style: AlertStyle,buttonTitle: String,buttonColor: UIColor,action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlerts {
        _ = showAlert(title, style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
            nil)
        userAction = action
        return self
    }
    
    open func showAlert(_ title: String, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlerts {
        self.showAlert(title,  style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
    ...
    open func showAlert(_ title: String, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        ...
        if buttonTitle.isEmpty == false {
            ...
            button.titleLabel?.font =  Constatns.LatoRegular(13)
            button.addTarget(self, action: #selector(SweetAlerts.pressed(_:)), for: UIControlEvents.touchUpInside)
    ...
    open func showAlert2(_ title: String, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?,otherbuttonColor: UIColor, action: ((_ isOtherButton: Bool) -> Void)? = nil , action2: ((_ isOtherButton2: Bool) -> Void)? = nil) -> SweetAlerts {
        self.showAlert2(title, style: style, buttonTitle: buttonTitle, buttonColor: buttonColor, otherButtonTitle: otherButtonTitle, otherButtonColor: otherbuttonColor)
        userAction = action
        userAction2 = action2
        return self
    }
    ...
    open func showAlert2(_ title: String, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, otherButtonColor: UIColor?,action: ((_ isOtherButton: Bool) -> Void)? = nil,action2: ((_ isOtherButton: Bool) -> Void)? = nil) {
        userAction = action
        userAction2 = action2
        let window: UIWindow = UIApplication.shared.keyWindow!
        window.addSubview(view)
    ...
    open func showAlert3(_ title: String) -> SweetAlerts {
        _ =  showAlert3(title, subTitle : "", style: .none)
        return self
    }
    
    open func showAlert3(_ title: String, subTitle : String,style: AlertStyle) -> SweetAlerts {
        _ = showAlert3(title,  subTitle : subTitle, style: style, buttonTitle: NSLocalizedString("Okay", comment : ""))
        return self
        
    }
    
    open func showAlert3(_ title: String, subTitle : String,style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlerts {
        _ = showAlert3(title,   subTitle : subTitle,style: style, buttonTitle: buttonTitle,buttonColor: UIColor.colorFromRGB(0xAEDEF4))
        userAction = action
        return self
    }
    
    open func showAlert3(_ title: String, subTitle : String, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlerts {
        _ = showAlert3(title,  subTitle : subTitle,style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
            nil)
        userAction = action
        return self
    }
    
    open func showAlert3(_ title: String,subTitle : String, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlerts {
        self.showAlert3(title, subTitle : subTitle,style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
            otherButtonTitle,otherButtonColor: UIColor.red)
        userAction = action
        return self
    }
    ...
    open func showAlert3(_ title: String, subTitle : String,style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        ...
        window.bringSubview(toFront: view)
        view.frame = window.bounds
        self.setupContentView2()
        self.setupTitleLabel2()
